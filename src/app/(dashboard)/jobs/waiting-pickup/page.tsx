...
export default function JobsWaitingPickupPage() {
  ...
  const load = () => {
    setLoading(true);
    void Actions.getJobsWaitingPickup(fromDate, toDate)
  ...
  const summary = useMemo(
    () =>
      filteredJobs.reduce(
        (r, j) => {
          r.jobs += 1;
          r.pending += toNumber(j.pending_pickup_count);
          r.picked += toNumber(j.picked_count);
  ...
  return (
    <div className="space-y-5">
      ...
      <StatusStatGrid
        stats={[
          { label: "ຖ້ຽວທັງໝົດ", value: summary.jobs, icon: <FaRoute />, tone: "orange" },
        ...
        {loading ? (
          ...
          <div className="py-14 text-center">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {searchText.trim() ? "ບໍ່ພົບຂໍ້ມູນຕາມຄໍາຄົ້ນຫາ" : "ບໍ່ມີຖ້ຽວທີ່ລໍເບີກເຄື່ອງ"}
            </p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-white/30 dark:bg-white/5 border-b border-slate-200/30 dark:border-white/5">
                    ...
                    <th className="px-4 py-3 whitespace-nowrap text-left font-semibold text-slate-600 dark:text-slate-300">ວັນຈັດສົ່ງ / ສ້າງເມື່ອ</th>
                    <th className="px-4 py-3 whitespace-nowrap text-left font-semibold text-slate-600 dark:text-slate-300">ລົດ / ຄົນຂັບ</th>
                    <th className="px-4 py-3 whitespace-nowrap text-left font-semibold text-slate-600 dark:text-slate-300">ຄວາມຄືບໜ້າບິນ</th>
                    <th className="px-4 py-3 whitespace-nowrap text-left font-semibold text-slate-600 dark:text-slate-300">ສ້າງ / ອະນຸມັດ</th>
