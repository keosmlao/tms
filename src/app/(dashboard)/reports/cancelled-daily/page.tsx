...
export default function CancelledDailyReportPage() {
  return (
    <DailyStatusReport
      title="ລາຍງານການຍົກເລີກການຈັດສົ່ງປະຈຳວັນ"
      subtitle="Cancelled Bills · Daily"
      icon={<FaBan className="text-rose-300" size={18} />}
      tone="rose"
      filenamePrefix="cancelled-bills"
      showFinished
      showRemark
      ...
      fetcher={async (from, to) => {
        const raw = (await Actions.getReportCancelledDaily(from, to)) as {
          rows: ApiRow[];
          days: string[];
          departments: string[];
          transports: string[];
