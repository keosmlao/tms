...
async function getBillsPending(session, fromDate, toDate, transportCode) {
  ...
  const [detailRows, detailItemRows, totalRows, partialRows] = await Promise.all([
    query(`SELECT bill_no FROM public.odg_tms_detail WHERE bill_no = ANY($1::varchar[]) GROUP BY bill_no`, [billNos]),
    query(`SELECT bill_no FROM public.odg_tms_detail_item WHERE bill_no = ANY($1::varchar[]) GROUP BY bill_no`, [billNos]),
    query(
    ...
    query(
      `WITH item_bills AS (SELECT DISTINCT bill_no FROM public.odg_tms_detail_item WHERE bill_no = ANY($1::varchar[])),
        assigned AS (SELECT bill_no, item_code, COALESCE(SUM(selected_qty), 0)::numeric AS selected_qty
          FROM public.odg_tms_detail_item WHERE bill_no = ANY($1::varchar[])
          GROUP BY bill_no, item_code)
       SELECT d.doc_no AS bill_no,
         COUNT(*) FILTER (WHERE GREATEST(COALESCE(d.qty, 0)::numeric - COALESCE(a.selected_qty, 0)::numeric, 0) > 0)::int AS remaining_count,
         COALESCE(SUM(GREATEST(COALESCE(d.qty, 0)::numeric - COALESCE(a.selected_qty, 0)::numeric, 0)), 0)::numeric AS remaining_qty_total
  ...
  const partialByBill = new Map(
    partialRows.map((row) => [row.bill_no, {
      remaining_count: Number(row.remaining_count ?? 0),
      remaining_qty_total: Number(row.remaining_qty_total ?? 0),
    }])
  );
  ...
  const trans = transRaw
    .map((bill) => {
      const fallbackTotals = totalByBill.get(bill.doc_no) ?? { remaining_count: 0, remaining_qty_total: 0 };
      const remainingSummary = detailItemBills.has(bill.doc_no)
        ? partialByBill.get(bill.doc_no) ?? { remaining_count: 0, remaining_qty_total: 0 }
        : detailBills.has(bill.doc_no)
        ? { remaining_count: 0, remaining_qty_total: 0 }
        : fallbackTotals;

      return {
        ...bill,
        remaining_count: remainingSummary.remaining_count,
        remaining_qty_total: remainingSummary.remaining_qty_total,
        partial_delivery: detailItemBills.has(bill.doc_no) && remainingSummary.remaining_count > 0,
      };
    })
